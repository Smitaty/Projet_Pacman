...
public class PanelPacmanGame extends JPanel {
	...
	public PositionAgent get(PositionAgent pos, boolean isPacman) {
		if(isPacman) {
			for(PositionAgent pacman : this.pacmans_pos) {
				if(pos.getX() == pacman.getX() && pos.getY() == pacman.getY())
					return pacman;
			}
		}
		else {
			for(PositionAgent ghost : this.ghosts_pos) {
				if(pos.getX() == ghost.getX() && pos.getY() == ghost.getY())
					return ghost;
			}
		}
		return null;
	...
	public int getCptFood() {return cptFood;}
	
	public void setCptFood(int cpt) {
		this.cptFood = cpt;
	}
